...
class Heuristic(object):
    def __init__(self, basefile="/tmp/arxiv_v2/json_db.zip", test=False, batch_size=128, data_augmentation=False):
    # def __init__(self, basefile="/home/pfahler/arxiv_learning/subset_ml_train.zip", test=False):
        ...
        if (not test and "train" not in self.basefile) or (test and "test" not in self.basefile):
            ...
            if test:
                ...
                self.data = [x for x in self.data if os.path.basename(x).replace(".json", "") not in test_papers]
            # # print(self.data[:100])
